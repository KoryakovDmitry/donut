...
from donut.plot_utils import hconcat_resize_min
...
class BARTDecoder(nn.Module):
    ...
    def prepare_inputs_for_inference(self, input_ids: torch.Tensor, past_key_values=None, use_cache: bool = None,
...
class DonutModel(PreTrainedModel):
    ...
    def __init__(self, config: DonutConfig):
        ...
        self.final_w = self.config.input_size[1]
        self.final_h = self.config.input_size[0]
    ...
    def inference(
        ...
        if return_max_bbox and return_confs and return_tokens:
            if return_plot:
                super_imposed_raw_heatmap_imgs = dict()
                    ...
                    if isinstance(top_val, list):
                        ...
                        for item in top_val:
                            ...
                            for item_key, item_val in item.items():
                                ...
                                if return_plot:
                                    max_bbox, super_imposed_raw_heatmap_img, super_imposed_img = self.get_bbox_with_plot(
                                        img_np=deepcopy(img_np),
                                        decoder_output_cross_attentions=decoder_output.cross_attentions,
                                        tokens=tokens,
                                        orig_h=orig_h,
                                        orig_w=orig_w,
                                    ...
                                    super_imposed_raw_heatmap_imgs[item_key] = super_imposed_raw_heatmap_img
                                    super_imposed_imgs[item_key] = super_imposed_img
                                else:
                                    max_bbox = self.get_bbox_with_plot(
                                        decoder_output_cross_attentions=decoder_output.cross_attentions,
                                        tokens=tokens,
                                        orig_h=orig_h,
                                        orig_w=orig_w,
                                ...
                                temp_item_obj[item_key] = [text, conf, max_bbox]
                    ...
                    # elif top_key in nested_dict_set:
                        ...
                        for sub_key, sub_val in top_val.items():
                            ...
                            if return_plot:
                                ...
                                super_imposed_raw_heatmap_imgs[sub_key] = super_imposed_raw_heatmap_img
                                super_imposed_imgs[sub_key] = super_imposed_img
                            else:
                                max_bbox = self.get_bbox_with_plot(
                                    decoder_output_cross_attentions=decoder_output.cross_attentions,
                                    tokens=tokens,
                            ...
                            temp_item_obj[sub_key] = [text, conf, max_bbox]
                        ...
                        if return_plot:
                            ...
                            super_imposed_raw_heatmap_imgs[top_key] = super_imposed_raw_heatmap_img
                            super_imposed_imgs[top_key] = super_imposed_img
                        else:
                            max_bbox = self.get_bbox_with_plot(
                                decoder_output_cross_attentions=decoder_output.cross_attentions,
                                tokens=tokens,
    ...
    @classmethod
    def get_bbox_with_plot(self, decoder_output_cross_attentions, tokens, img_np=None, orig_w=None, orig_h=None,
                           final_h=1280, final_w=960,
                           discard_ratio=0.5,
                           return_plot=False):

        if return_plot:
            max_bbox, thres_heatmap, agg_heatmap = DonutModel.max_bbox_from_heatmap(
                decoder_cross_attentions=decoder_output_cross_attentions,
                tkn_indexes=tokens, final_h=final_h, final_w=final_w, discard_ratio=discard_ratio,
                return_thres_agg_heatmap=True,
            )

            raw_heatmap = deepcopy(agg_heatmap)
            raw_image = deepcopy(img_np)

            x1, y1, x2, y2 = max_bbox
            img_np = cv2.rectangle(img_np, (x1, y1), (x2, y2), (255, 100, 200), 2)

            heatmap_img = cv2.applyColorMap(thres_heatmap, cv2.COLORMAP_JET)
            super_imposed_img = cv2.addWeighted(heatmap_img, 0.5, img_np, 0.5, 0)

            super_imposed_raw_heatmap_img = cv2.addWeighted(cv2.applyColorMap(raw_heatmap, cv2.COLORMAP_JET), 0.5,
                                                            raw_image, 0.5, 0)

            scale_x = orig_w / final_w
            max_bbox[0] = scale_x * max_bbox[0]
            max_bbox[2] = scale_x * max_bbox[2]

            scale_y = orig_h / final_h
            max_bbox[1] = scale_y * max_bbox[1]
            max_bbox[3] = scale_y * max_bbox[3]

            return max_bbox, super_imposed_raw_heatmap_img, super_imposed_img
        else:
            max_bbox = DonutModel.max_bbox_from_heatmap(
                decoder_cross_attentions=decoder_output_cross_attentions,
                tkn_indexes=tokens, final_h=final_h, final_w=final_w, discard_ratio=discard_ratio,
                return_thres_agg_heatmap=False,
            )
